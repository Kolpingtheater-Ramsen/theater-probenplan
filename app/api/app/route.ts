...
function forbidden() {
  return NextResponse.json(
    { error: 'Dafür ist eine Adminrolle erforderlich.' },
    { status: 403 },
  );
}
...
export function GET(request: NextRequest) {
  ...
  const absences = db
    .prepare(
      'SELECT id, date_from AS "from", date_to AS "to", reason FROM absences WHERE user_id = ? ORDER BY date_from',
    )
    .all(user.userId);
  const organizationAbsenceCount =
    user.role === 'admin'
      ? (
          db.prepare('SELECT COUNT(*) AS count FROM absences').get() as {
            count: number;
          }
        ).count
      : absences.length;
  ...
  const accounts =
    ...
    ORDER BY CASE p.role WHEN 'admin' THEN 0 ELSE 1 END, p.display_name`)
          .all()
      : [];
  const poll = db
    .prepare(
      "SELECT confirmed_option_id AS confirmedOptionId FROM polls WHERE id = 'maskenball'",
    )
    .get() as { confirmedOptionId: string | null };
  const vote = db
    .prepare(
      "SELECT option_id AS optionId FROM poll_votes WHERE poll_id = 'maskenball' AND user_id = ?",
    )
    .get(user.userId) as { optionId: string } | undefined;
  const pollOptions = db
    .prepare(`SELECT o.id, o.day_label AS day, o.time_label AS time, o.votes_seed + COUNT(v.user_id) AS votes
    FROM poll_options o LEFT JOIN poll_votes v ON v.option_id = o.id WHERE o.poll_id = 'maskenball' GROUP BY o.id ORDER BY o.rowid`)
    .all();
  const userSettings = db
    .prepare(
      'SELECT reminders_json AS remindersJson FROM user_settings WHERE user_id = ?',
    )
    .get(user.userId) as { remindersJson: string } | undefined;
  ...
  return NextResponse.json({
    ...
    reminders: userSettings
      ? JSON.parse(userSettings.remindersJson)
      : remindersDefault,
    organization,
  });
}
...
const actionSchema = z.discriminatedUnion('action', [
  ...
  z.object({
    action: z.literal('absence.create'),
    from: z.iso.date(),
    to: z.iso.date(),
    reason: z.string().trim().max(500).default(''),
  }),
  z.object({
    action: z.literal('absence.delete'),
    id: z.string().min(1).max(100),
  }),
  z.object({
    action: z.literal('poll.vote'),
    optionId: z.enum(['fri', 'sat', 'sun']),
  }),
  ...
  z.object({
    action: z.literal('checkin.save'),
    eventId: z.string().min(1).max(100),
    members: z
      .array(
        z.object({ id: z.number().int().positive(), present: z.boolean() }),
      )
      .max(300),
  }),
  z.object({
    action: z.literal('settings.reminders'),
    value: z.object({
      dayBefore: z.boolean(),
      twoHours: z.boolean(),
      changes: z.boolean(),
  ...
  z.object({
    action: z.literal('settings.organization'),
    value: z.object({
      ...
      groupVisibility: z.object({
        techOnly: z.boolean(),
        costumeOnly: z.boolean(),
      }),
    }),
  }),
  z.object({
    action: z.literal('reminders.send'),
    eventId: z.string().min(1).max(100).default('weekly-03'),
  }),
  z.object({
    ...
    group: z.string().trim().min(1).max(80),
    roleName: z.string().trim().max(80).default(''),
    profileRole: z.enum(['member', 'admin']).default('member'),
  }),
...
export async function POST(request: NextRequest) {
  ...
  if (input.action === 'attendance') {
    const event = db
      .prepare('SELECT locked FROM events WHERE id = ?')
      .get(input.eventId) as { locked: number } | undefined;
    if (!event)
      return NextResponse.json(
    ...
    const result = db
      .prepare('DELETE FROM absences WHERE id = ? AND user_id = ?')
      .run(input.id, user.userId);
    if (!result.changes)
      return NextResponse.json(
    ...
    db.prepare(`INSERT INTO user_settings (user_id, reminders_json) VALUES (?, ?)
      ON CONFLICT(user_id) DO UPDATE SET reminders_json = excluded.reminders_json, updated_at = CURRENT_TIMESTAMP`).run(
      user.userId,
    ...
    const poll = db
      .prepare(
        "SELECT confirmed_option_id AS confirmed FROM polls WHERE id = 'maskenball'",
      )
      .get() as { confirmed: string | null };
    if (poll.confirmed)
      return NextResponse.json(
        { error: 'Die Abstimmung ist bereits beendet.' },
        { status: 409 },
      );
    db.prepare(`INSERT INTO poll_votes (poll_id, option_id, user_id) VALUES ('maskenball', ?, ?)
      ON CONFLICT(poll_id, user_id) DO UPDATE SET option_id = excluded.option_id, updated_at = CURRENT_TIMESTAMP`).run(
      input.optionId,
      user.userId,
    );
    ...
    if (input.action === 'poll.confirm') {
      const winner = db
        .prepare(`SELECT o.id FROM poll_options o LEFT JOIN poll_votes v ON v.option_id = o.id WHERE o.poll_id = 'maskenball'
        GROUP BY o.id ORDER BY o.votes_seed + COUNT(v.user_id) DESC, o.id LIMIT 1`)
        .get() as { id: string };
      db.prepare(
        "UPDATE polls SET confirmed_option_id = ? WHERE id = 'maskenball'",
      ).run(winner.id);
      if (winner.id === 'sat')
        db.prepare(`INSERT OR IGNORE INTO events (id, day, month, weekday, title, time_label, place, group_name, people, kind, tone, starts_at, ends_at, created_by)
        VALUES ('poll-result', 19, 'SEP', 'Samstag', 'Zusatzprobe · Maskenball', '14:00–17:00', 'Kolpingheim · Großer Saal', 'Alle Getaggten', 19, 'other', 'blue', '2026-09-19T14:00:00+02:00', '2026-09-19T17:00:00+02:00', ?)`).run(
          user.userId,
        );
      ...
      db.prepare(`INSERT INTO events (id, day, month, weekday, title, time_label, place, group_name, people, kind, tone, is_custom, starts_at, ends_at, created_by)
        VALUES (?, ?, ?, ?, ?, ?, 'Kolpingheim · Großer Saal', ?, 0, 'other', 'violet', 1, ?, ?, ?)`).run(
        id,
        day,
        month,
        weekday,
      ...
      const result = db
        .prepare('DELETE FROM events WHERE id = ? AND is_custom = 1')
        .run(input.id);
      if (!result.changes)
        return NextResponse.json(
          { error: 'Termin nicht gefunden oder nicht löschbar.' },
          { status: 404 },
        );
      ...
      db.transaction(() => {
        const statement =
          db.prepare(`INSERT INTO checkins (event_id, member_id, present, recorded_by) VALUES (?, ?, ?, ?)
      ...
      db.prepare(`INSERT INTO organization_settings (id, settings_json) VALUES (1, ?)
        ON CONFLICT(id) DO UPDATE SET settings_json = excluded.settings_json, updated_at = CURRENT_TIMESTAMP`).run(
        JSON.stringify(input.value),
      );
      ...
      try {
        db.transaction(() => {
          ...
          db.prepare(
            'INSERT INTO members (email, name, group_name, role_name, initials) VALUES (?, ?, ?, ?, ?)',
        ...
        return NextResponse.json(
          {
            error:
              'E-Mail ist bereits vergeben oder das Mitglied konnte nicht angelegt werden.',
          },
          { status: 409 },
        );
      }
      ...
      if (input.userId === user.userId)
        return NextResponse.json(
          {
            error: 'Das eigene Passwort kann hier nicht zurückgesetzt werden.',
          },
          { status: 400 },
        );
      const target = db
      ...
      db.transaction(() => {
        db.prepare(
          'UPDATE profiles SET password_hash = ?, failed_attempts = 0, locked_until = NULL, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?',
        ...
        db.prepare(`INSERT INTO temporary_credentials (user_id, temporary_password) VALUES (?, ?)
          ON CONFLICT(user_id) DO UPDATE SET temporary_password = excluded.temporary_password, created_at = CURRENT_TIMESTAMP`).run(
          input.userId,
          temporaryPassword,
        );
