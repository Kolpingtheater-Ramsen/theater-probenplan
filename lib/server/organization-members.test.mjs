...
test('references an optimized local portrait when a public team photo exists', () => {
  const portraits = organizationMemberSeeds.filter((member) => member.avatar);
  assert.equal(portraits.length, 37);
  for (const member of portraits)
    assert.equal(
      existsSync(`public${member.avatar}`),
      true,
      `missing portrait for ${member.name}`,
    );
});
