...
export function getAuthUser(request: NextRequest): AuthUser | null {
  ...
  const row = getDatabase()
    .prepare(`SELECT p.user_id AS userId, p.email, p.display_name AS displayName, p.role,
    ...
    | (Omit<AuthUser, 'mustChangePassword'> & { mustChangePassword: number })
    | undefined;
  return row
    ? { ...row, mustChangePassword: Boolean(row.mustChangePassword) }
    : null;
}
...
export function sameOrigin(request: NextRequest) {
  ...
  return originMatchesHosts(origin, [
